...
class RequestList extends StatelessWidget {
  ...
  ElevatedButton(
    ...
    onPressed: () async {
      final currentUser = FirebaseAuth.instance.currentUser!;
      try {
        // 1️⃣ Create a new approval record so it shows up under "Request Approvals"
        await FirebaseFirestore.instance
            .collection('approvals')
            .add({
              'requestId':  requestId,
              'receiverId': request['userId'],   // original requester (A)
              'donorId':    currentUser.uid,     // approver (B)
              'timestamp':  FieldValue.serverTimestamp(),
              'confirmed':  false,
              'type':       'request_approved',
            });

        // 2️⃣ Push into the chat screen immediately
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => ChatPage(
