// lib/widgets/AnnouncementsPage.dart
...
class _AnnouncementsPageState extends State<AnnouncementsPage> {
  ...
  Future<void> _uploadAnnouncement() async {
    ...
    try {
      String imageUrl = "";
      if (_imageBytes != null) {
        final ref = FirebaseStorage.instance.ref().child('announcements/$_imageName');
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Announcement posted successfully.")));
  ...
  Future<void> _deleteAnnouncement(String docId, String? imageUrl) async {
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Announcement deleted successfully.")));
    } catch (e) {
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text("Error deleting announcement: $e")));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Announcements"), backgroundColor: Colors.blue),
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 10),
          ElevatedButton.icon(onPressed: _pickImage, icon: const Icon(Icons.image), label: const Text("Select Image (Optional)")),
          const SizedBox(height: 10),
          if (_imageBytes != null) Image.memory(_imageBytes!, height: 150, fit: BoxFit.cover),
          const SizedBox(height: 20),
          ElevatedButton(
            onPressed: _isUploading ? null : _uploadAnnouncement,
            child: _isUploading ? const CircularProgressIndicator() : const Text("Post Announcement"),
          ),
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                        IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _editAnnouncement(doc)),
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (confirm == true) {
                              await _deleteAnnouncement(doc.id, data['image_url'] as String?);
                            }
                          },
                        ),
                      ]),
